...
const Game = (() => {
  ...
  function _initVsPlayers() {
    ...
    cfg1.x = _spawnX(0, 2 + sessionConfig.extraBots + (sessionConfig.customP2IsNPC ? 0 : 1));
    ...
    if (!sessionConfig.customP2IsNPC) {
      ...
      cfg2.x = _spawnX(1, 1 + sessionConfig.extraBots + 1);
      cfg2._respawnX = cfg2.x; cfg2._respawnY = WORLD_H - 200;
      const bot = new Player(cfg2);
    ...
    for (let i = 0; i < sessionConfig.extraBots; i++) {
      ...
      const totalPlayers = 1 + (sessionConfig.customP2IsNPC ? 1 : 1) + sessionConfig.extraBots;
      const botCfg = _getCharConfig(charDef.id, 100 + i, i % 2 === 0 ? -1 : 1, null, {
        isNPC: true, aiLevel: botDiff,
        color: BOT_COLORS[i % BOT_COLORS.length],
      });
      botCfg.x = _spawnX(spawnIdx, totalPlayers);
